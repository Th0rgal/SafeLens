...
export default function SettingsPage() {
  ...
  // Local draft — edits happen here, persisted only on Save
  const [draft, setDraft] = useState<SettingsConfig | null>(null);
  // Chains stored as ordered array to prevent JS numeric key auto-sorting
  ...
  // Build the full draft config with chains from the ordered array
  ...
  const handleReset = () => {
    resetConfig();
    setDraft(null); // will re-sync from context on next render
    setChainEntries([]);
  };
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "safelens-settings.json";
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const imported = importSettingsConfig(text);
      setDraft(imported);
