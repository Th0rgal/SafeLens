...
/** Display a hash value with a copy-to-clipboard button. */
export function HashValueDisplay({ hash }: { hash: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <code className="flex-1 break-all text-xs font-mono">{hash}</code>
      <button onClick={copy} className="text-xs text-accent hover:text-accent-hover flex items-center gap-1">
    ...
    <div className="space-y-2">
      ...
      {expanded && (
        <div className="mt-2 space-y-3 rounded-md border border-border/15 glass-subtle p-3">
          ...
          <div>
            <div className="mb-1 text-xs font-medium text-muted">Domain Separator</div>
            <div className="flex items-center gap-2">
              <code className="flex-1 break-all text-xs font-mono">{details.domainSeparator}</code>
              <button onClick={() => copyToClipboard(details.domainSeparator, "domainSeparator")} className="text-xs text-accent hover:text-accent-hover shrink-0">
                {copiedField === "domainSeparator" ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
              </button>
            </div>
            <div className="mt-1 text-xs text-muted/70">EIP-712 domain separator (chainId + Safe address)</div>
          </div>
          <div>
            <div className="mb-1 text-xs font-medium text-muted">Message Hash</div>
            <div className="flex items-center gap-2">
              <code className="flex-1 break-all text-xs font-mono">{details.messageHash}</code>
              <button onClick={() => copyToClipboard(details.messageHash, "messageHash")} className="text-xs text-accent hover:text-accent-hover shrink-0">
                {copiedField === "messageHash" ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
              </button>
            </div>
            <div className="mt-1 text-xs text-muted/70">SafeTx struct hash (transaction parameters)</div>
          </div>
